...
function loadCSS(){
    if (!document.getElementById("carouselCSS")) {
        ...
        console.error("Le CSS du carousel n'a pas été chargé initialement et a dû l'être par le script.\nN'oublie pas de rajouter la balise Link");
        console.error(link);
    }
}
...
function slide(className, offset) {
    ...
    container.style.transition = `transform ${slidingTime/globalOffset}ms ease-in-out`;
...
function moveToSlide(slideIndex) {
    globalOffset = slideIndex - activeSlide;
    let className = "";
    if (globalOffset === 0) {
        globalOffset = 1;
        return;
    ...
    globalOffset = Math.abs(globalOffset);
    slide(className, globalOffset);
}
